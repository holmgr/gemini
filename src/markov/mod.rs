...
impl MarkovChain {
    ...
    fn train(&mut self, input: &String) {
        ...
        fn increment_edge(first_node: &NodeIndex,
        ...
        while let Some(current_char) = chars.next() {
            let next_node = self.graph
                ...
                .find(|node| self.graph.node_weight(*node) == Some(&current_char.to_string()))
                .unwrap();
    ...
    fn normalize(&mut self) {
        ...
        while let Some(node) = bfs.next(&self.graph) {
            let edge_sum =
                match self.graph.edges(node).fold(0.0, |sum, edge| sum + *edge.weight() as f64) {
                    x if x > 0.0 => x,
                    _ => 1.0,
                };
...
mod tests {
    ...
    fn test_train() {
        ...
        assert_eq!(markov.graph
                       .edge_references()
                       .fold(0.0, |sum, edge| sum + *edge.weight() as f64),
                   training_str.len() as f64 + 1.0);
    }

    #[test]
    fn test_normalize() {
        let starting_strings = vec![String::from("ABC"), String::from("ACC"), String::from("ACD")];
        ...
        assert!(markov.graph.raw_edges().iter().any(|ref edge| {
            ...
            edge.target() == NodeIndex::new(2 + 2 * 26 + 2) && edge.weight == 1.0 / 2.0
        }));
        assert!(markov.graph.raw_edges().iter().any(|ref edge| {
            edge.source() == NodeIndex::new(2 + 26 + 2) &&
            edge.target() == NodeIndex::new(2 + 2 * 26 + 3) && edge.weight == 1.0 / 2.0
        }));
        assert!(markov.graph.raw_edges().iter().any(|ref edge| {
    ...
    #[test]
    fn test_generate() {
        let starting_strings =
            vec![String::from("ABCD"), String::from("ACCC"), String::from("ACDC")];
        let mut markov = MarkovChain::new(&starting_strings, DEFAULT_SEED);
